...
import persistence.meta.constraint.MetaDataColumnConstraint;
import persistence.meta.relation.MetaDataColumnEmptyRelation;
import persistence.meta.relation.MetaDataColumnRelation;
...
public class MetaDataColumn {
  ...
  private MetaDataColumn(String name, String type, String fieldName, Field field,
      List<MetaDataColumnConstraint> constraints,
  ...
  public static MetaDataColumn of(Field field, String columnType) {
    ...
    boolean isRelation = Arrays.stream(field.getAnnotations())
        .anyMatch(annot -> annot.annotationType().equals(OneToMany.class) || annot.annotationType()
            .equals(JoinColumn.class));

    Relation relation =
        isRelation ? MetaDataColumnRelation.of(field) : new MetaDataColumnEmptyRelation();

    return new MetaDataColumn(getDBColumnName(field), columnType, field.getName(), field,
