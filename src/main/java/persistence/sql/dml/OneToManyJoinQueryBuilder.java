...
public class OneToManyJoinQueryBuilder extends QueryBuilder {
    ...
    private String generateJoinQuery(EntityMeta entityMeta, int depth) {
        ...
        final String joinTableSignature = tableNameSignature(oneToManyAssociate.getManyEntityMeta().getTableName(),
                depth + 1);

        return dialect.leftJoin(oneToManyAssociate.getManyEntityMeta().getTableName(), joinTableSignature) +
                columnSignature(drivenTableSignature, joinPkColumn.getName()) + " = " + columnSignature(
                joinTableSignature, oneToManyAssociate.foreignerColumnName());
    }
